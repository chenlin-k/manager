(function($,undefined){
	$.fn.uploadInit = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "http://111.230.91.253:8080/WordOcr/upload",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="upload_btn">开始识别</div>';
		            html += '				</div>';
		            html += '			</div>';
					html += '			<div id="preview" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">确认图片文件</button>';
					html += '		</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<h2 style=\"font-family:  'STKaiti';\">所有图片转换完成：</h2><br/><p style=\"font-family:  'STKaiti';font-size:20px;font-weight:bold \">图片"+ index +"文本信息：</p><p></br>" + da + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
